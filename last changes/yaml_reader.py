...
try:
    with open("synonyms.yaml", "r") as file:
        yaml.safe_load(file)
except:
    basic_synonyms = {"ydx": {'synonym_name': 'yandex', 'synonym_value': 'https://yandex.by',
                              'created': datetime.now().strftime('%Y-%m-%d %H:%M:%S')},
                      "ggl": {'synonym_name': 'google', 'synonym_value': 'https://google.by',
                              'created': datetime.now().strftime('%Y-%m-%d %H:%M:%S')},
                      "yt": {'synonym_name': 'youtube', 'synonym_value': 'https://www.youtube.com/',
                             'created': datetime.now().strftime('%Y-%m-%d %H:%M:%S')}}
    # create synonyms.yaml if not exists
    with open("synonyms.yaml", "w") as file:
        yaml.dump(basic_synonyms, file)
