...
while True:
    ...
    print("executing... ")
    d = Database()
    if re.search('tagcounter --get [a-z:/]+?[a-zA-Z0-9_.-]+\.[a-zA-Z]?[a-zA-Z]?[a-zA-Z]?[a-zA-Z]?', _input):
        end = re.search('[a-zA-Z0-9_.-]+$', _input)
        arg = end.group()
        t = Tag_counter(arg)
        tags = t.tags_to_dict()
        sitename = t.site_name()
        d.insert(tags, sitename, url=t.HOST)
        print(tags)
    elif re.search('tagcounter --view [a-z:/]+?[a-zA-Z0-9_.-]+\.[a-zA-Z]?[a-zA-Z]?[a-zA-Z]?[a-zA-Z]?', _input):
        end = re.search('[a-zA-Z0-9_.-]+$', _input)
        arg = end.group()
        t = Tag_counter(arg)
        tags = d.get_from_db(t.HOST)
        if tags is None:
            print(f"Data from {t.HOST} doesn't exist in the database")
        else:
            print(tags)
    else:
        window = Tk()
